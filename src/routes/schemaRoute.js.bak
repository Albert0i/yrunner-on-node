...
const { convertToSQL } = require('../utils/convertToSQL')
...
router.post('/:table/sql', verifyPassphrase, async (req, res) => {
    const table = req.params.table
    const sql = schemaSQL.replace('myOwner', dbConfig.user).replace('myTable', table)
    const result = await runSelectSQL(sql, true)

    if (result.success)
        res.status(200).json({ success: true, sql: convertToSQL(table, result.rows)})
    else 
